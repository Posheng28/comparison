...
const cache = new Map<string, { at: number; map: Record<string, [number, number, number]> }>()
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const legalShares: Record<string, number | null> = {} // 三大法人合計持股(張)
    for (const d of dates) {
      ...
      legalShares[d] = k ? map[k][2] : null
    }
    return NextResponse.json({ code, foreign, legal, legalShares })
  } catch (e) {
